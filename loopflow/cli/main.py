...
LoopFlow 命令行工具，提供完整的 Loop Engineering 子命令链：
...
from loopflow.cli.audit import audit
from loopflow.cli.cost import cost
from loopflow.cli.init import init
from loopflow.cli.run import run
from loopflow.cli.service import service
from loopflow.cli.skill import skill
from loopflow.cli.state import state
from loopflow.cli.worktree import worktree


@click.group()
@click.version_option(version=__version__, prog_name="loopflow")
def cli() -> None:
    """LoopFlow 命令行工具 — LoopFlow = Loops × WorkFlows + Agents & Services."""


@cli.command()
def hello() -> None:
    """占位命令，验证 CLI 入口可用。"""
    click.echo(f"LoopFlow v{__version__} — LoopFlow 框架已就绪。")
