...
从 LoopFlow 的 :class:`Pattern` 生成符合 Claude Code Skill 规范的 ``SKILL.md``
文档，并产出主 Skill ``using-loopflow``（会话启动注入，强制使用相关模式）。

设计要点：

- :class:`SkillConfig` 用 pydantic 描述 Skill 的可结构化字段（名称、描述、触发
  条件、步骤、验证、约束）。
- :class:`SkillGenerator` 通过模式注册表 (:mod:`loopflow.patterns.registry`)
...
- ``generate_master_skill()`` 参考 superpowers 的 using-superpowers，用强制语气
  声明"如有相关 LoopFlow 模式适用当前任务，必须使用，不可跳过"。

用法::

    from loopflow.skill import SkillGenerator
...
from loopflow.patterns.base import Pattern
...
MASTER_SKILL_NAME = "using-loopflow"
...
class SkillGenerator:
    ...
    def generate_master_skill(self) -> str:
        """生成主 Skill ``using-loopflow``。

        参考 superpowers 的 using-superpowers，用强制语气声明：会话启动时自动
        加载；如果 LoopFlow 模式适用当前任务，必须使用，不可跳过。
        """
        # 列出所有可用模式及其触发场景
        ...
        description = (
            ...
            "- establishes how to discover and run LoopFlow patterns, requiring "
            "pattern invocation before acting on relevant tasks"
        )

        body = f"""# Using LoopFlow

<EXTREMELY-IMPORTANT>
If you think there is even a 1% chance a LoopFlow pattern might apply to what you are doing, you ABSOLUTELY MUST invoke the pattern.
...
**Invoke relevant LoopFlow patterns BEFORE any response or action** — including
...
**Before entering implementation:** if a LoopFlow pattern applies, audit project
readiness first, then run the loop with the appropriate trust level (L1 report /
L2 assist / L3 autonomous).

Then announce "Using LoopFlow pattern `<name>` to <purpose>" and follow the
