...
继承 :class:`loopkits.core.agent.Corrector`，扩展为 Work 模式的校正器。
...
from loopkits.core.agent import Corrector
...
class WorkCorrector(Corrector):
    ...
    继承 :class:`~loopkits.core.agent.Corrector`，复用其 ``confidence_threshold``
    ...
    def execute(self, task: Any) -> CorrectorAction:
        """兼容 :class:`~loopkits.core.agent.Agent` 接口。
